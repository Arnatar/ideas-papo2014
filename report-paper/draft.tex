...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{#define malloc_idea_matrix(name)                              \textbackslash{}}
    \NormalTok{Idea **name = (Idea **)malloc(num_rows * }\KeywordTok{sizeof}\NormalTok{(Idea *)); \textbackslash{}}
    \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{i = }\DecValTok{0}\NormalTok{; i < num_rows; ++i)                        \textbackslash{}}
        \NormalTok{name[i] = (Idea *)malloc(num_cols * }\KeywordTok{sizeof}\NormalTok{(Idea));    \textbackslash{}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{malloc_idea_matrix(field)}
\NormalTok{malloc_idea_matrix(field_new)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{#define copy_field_into_field_new()        \textbackslash{}}
    \NormalTok{for_every(i, num_rows, \{               \textbackslash{}}
        \NormalTok{for_every(j, num_cols, \{           \textbackslash{}}
            \NormalTok{field_new[i][j] = field[i][j]; \textbackslash{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{#define send_ideas(ideas_arr, to, tag, req) \textbackslash{}}
  \NormalTok{MPI_Isend(ideas_arr, num_cols, mpi_idea_type, to, tag, MPI_COMM_WORLD, &req) }

\OtherTok{#define receive_ideas_into(ideas_arr, from, tag, req) \textbackslash{}}
  \NormalTok{MPI_Irecv(ideas_arr, num_cols, mpi_idea_type, from, tag, MPI_COMM_WORLD, &req) }

\OtherTok{#define send_top_rows(field) \textbackslash{}}
  \CommentTok{/* send our first ghost row into the bottom real row of the previous rank */} \NormalTok{\textbackslash{}}
  \NormalTok{send_ideas(field[}\DecValTok{0}\NormalTok{], prev_rank, GHOST, req);                                \textbackslash{}}
  \CommentTok{/* send our first real row into the bottom ghost row of the previous rank */} \NormalTok{\textbackslash{}}
  \NormalTok{send_ideas(field[}\DecValTok{1}\NormalTok{], prev_rank, REAL, req2);                                 \textbackslash{}}

\OtherTok{#define receive_into_bottom_rows(field) \textbackslash{}}
  \CommentTok{/* receive first ghost row from next rank into our bottom real row */} \NormalTok{\textbackslash{}}
  \NormalTok{receive_ideas_into(field[num_rows}\DecValTok{-2}\NormalTok{], next_rank, GHOST, req);        \textbackslash{}}
  \CommentTok{/* receive first real row from next rank into our bottom ghost row */} \NormalTok{\textbackslash{}}
  \NormalTok{receive_ideas_into(field[num_rows}\DecValTok{-1}\NormalTok{], next_rank, REAL, req2);         \textbackslash{}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{#define mpi_define_idea_type()                                               \textbackslash{}}
  \DataTypeTok{int}          \NormalTok{blocklengths[}\DecValTok{5}\NormalTok{] = \{}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{\};                                  \textbackslash{}}
  \NormalTok{MPI_Datatype types[}\DecValTok{5}\NormalTok{] = \{MPI_INT,MPI_INT,MPI_INT, MPI_INT, MPI_INT\};               \textbackslash{}}
  \NormalTok{MPI_Datatype mpi_idea_type;                                                \textbackslash{}}
  \NormalTok{MPI_Aint     offsets[}\DecValTok{5}\NormalTok{];                                                   \textbackslash{}}
  \NormalTok{offsets[}\DecValTok{0}\NormalTok{] = offsetof(Idea, a);                                            \textbackslash{}}
  \NormalTok{offsets[}\DecValTok{1}\NormalTok{] = offsetof(Idea, b);                                            \textbackslash{}}
  \NormalTok{offsets[}\DecValTok{2}\NormalTok{] = offsetof(Idea, c);                                            \textbackslash{}}
  \NormalTok{offsets[}\DecValTok{3}\NormalTok{] = offsetof(Idea, h);                                            \textbackslash{}}
  \NormalTok{offsets[}\DecValTok{4}\NormalTok{] = offsetof(Idea, empty);                                        \textbackslash{}}
    \NormalTok{MPI_Type_create_struct(}\DecValTok{5}\NormalTok{, blocklengths, offsets, types, &mpi_idea_type); \textbackslash{}}
    \NormalTok{MPI_Type_commit(&mpi_idea_type);                                         \textbackslash{}}
\end{Highlighting}
\end{Shaded}
